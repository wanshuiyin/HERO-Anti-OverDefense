...
不留任何生成/纠偏痕迹:最终产出(代码、注释、docstring、commit message、
...
Same eight rules, no worked examples — roughly half the size. Use it when your
...
No generation or correction traces: deliverables show only the correct final
...
不留生成/纠偏痕迹:产出只呈现正确的最终结果——不提报错、废方案、
